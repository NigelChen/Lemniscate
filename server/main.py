import socket, threading,time, os.path, random
#server code, protects against any clients controlling server controls
serverCode = random.randrange(23213,999999)
#Checks if there is a config file
if not os.path.exists('config.txt'):
    open('config.txt','a')
    f = open('config.txt','w')
    f.write('#HOST (Your ipv4 address):')
    f.write('\n')
    f.write('127.0.0.1')
    f.write('\n')
    f.write('#PORT:')
...
def userUpdate():
    ...
    global users
    for i in range (0,len(users)):
        users[i].send(str(serverCode)+'userUpdate')
    time.sleep(0.2)
    ...
    time.sleep(0.3)
    for i in range (0,len(users)):
        users[i].send(str(serverCode)+'stop')
...
while 1:
    while 1:
        ...
        if not banned:
            ...
            continue
    #Sends the server code over.
    c.send(str(serverCode))
    c.send('Please enter your name:')
    try:
        name = c.recv(1024)
    except:
        print 'Client logged off before entering.'
    f.close()
    if name in rawNames:
